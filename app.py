...
- Երբ քեզ ուղարկում են նկար, մանրամասն վերլուծիր այն և պատասխանիր օգտատիրոջ հարցին:
"""
...
def get_new_chat_object(history_messages=[]):
    ...
    # Ավելացված է Google Search գործիքը՝ համացանցից որոնումների համար
    ...
    for cid in sorted_chat_ids:
        ...
        with col_opt:
            with st.popover("⋮"):
                ...
                pin_label = (
                    "📌 Ապաամրացնել" if chat_data.get("pinned") else "📌 Ամրացնել"
                )
                ...
                if st.button("✏️ Վերանվանել", key=f"rename_btn_{cid}"):
                    if new_title.strip():
                        chat_data["title"] = new_title.strip()
                        save_chat_to_db(cid)
                        st.rerun()

                if st.button("🗑️ Ջնջել", key=f"del_{cid}"):
...
for idx, message in enumerate(active_chat["messages"]):
    with st.chat_message(message["role"]):
        if message.get("image_url"):
            st.image(
                message["image_url"], caption="Գեներացված նկար", use_container_width=True
            )
    ...
    with st.chat_message("assistant"):
        with st.spinner("Մտածում եմ..."):
            ...
            if is_image_request:
                ...
                image_url = f"https://pollinations.ai/p/{encoded_prompt}?width=1024&height=1024&seed={random_seed}&model=flux"

                st.image(
                    image_url, caption="Ահա ձեր նկարը 🎨", use_container_width=True
                )
                ...
                if not success:
                    error_msg = "⚠️ Սերվերը ժամանակավորապես ծանրաբեռնված է (503): Խնդրում ենք փորձել 1-2 րոպեից:"
